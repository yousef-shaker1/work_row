...
class CheckResource extends Resource
{
    ...
    protected function getTableQuery(): Builder
    {
        return parent::getTableQuery()
            ->where('employee_id', Auth::guard('employee')->user()->id); // Replace 'status' and 'active' with your condition
    }
    ...
    public static function form(Form $form): Form
    {
        ...
        return $form
            ->schema([
                ...
                TimePicker::make('check_in')->default(now()->addHours(3)),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->headerActions([
                ...
                ->label('Start Break')
                ->action(function () {
                    if (!session()->has('breaks')) {
                        session()->put('breaks', 0);
                    }
                    if (!self::todayCheck()) {
                        self::sendNotifications('Error!', 'No Breaks Allowed Out Of The Shift!');
                        return;
